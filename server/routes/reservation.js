...
router.post('/', [
    ...
    const sql = `INSERT INTO reservations (name, email, phone, date, time, guests, special_requests, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))`;
    ...
    db.run(sql, [name, email, phone, date, time, guests, specialRequests || ''], function(err) {
        ...
        res.status(201).json({ message: 'Reservation successful!', reservationId: this.lastID });
    });
});
